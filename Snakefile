...
rule repeatmasker:
    ...
    output:
        repeatmasker_output=f"repeat_masker_outputs/{config['fasta_file']}.out.gff"
    params:
    ...
    singularity:
        "images/dfam_tetools.sif"
    shell:
        "RepeatMasker -gff -a -pa {params.repeatmasker_threads} -lib {params.volume_name}/{input.repeatmodeler_output} {params.volume_name}/{input.fasta_file} -dir {params.volume_name}/repeat_masker_outputs"
...
rule pbmm2:
    ...
    shell:
#        "pbmm2 align --preset ISOSEQ -O6,24 -B4 --sort {params.volume_name}/{input.isoseq3_reads} {params.volume_name}/{input.bedtools_output} {params.volume_name}/{output.pbmm2_output} -J 4 -j {params.pbmm2_threads}"
        r"""
        set -euo pipefail
        export TMPDIR="{resources.tmpdir}"
        mkdir -p "$(dirname "{params.volume_name}/{output.pbmm2_output}")"
        pbmm2 align --preset ISOSEQ -O6,24 -B4 --sort \
          "{params.volume_name}/{input.bedtools_output}" \
          "{params.volume_name}/{input.isoseq3_reads}" \
          "{params.volume_name}/{output.pbmm2_output}" \
          -J 4 -j {params.pbmm2_threads}
        """
...
rule maker_control_files:
    ...
    params:
         fasta_file = f"bedtools_outputs/{species_name}.softmask.fasta",
         isoseq3_output = f"isoseq_outputs/{species_name}.collapsed.gff",
...
#        tail -n +2 maker_outputs/*.gff | awk -F"\t" 'NF==9 && ($3=="gene" || $3 =="CDS" || $3 =="mRNA" || $3 =="exon" || $3 == "five_prime_UTR" || $3 == "three_prime_UTR" || $3 =="tRNA" )' >> {output}
...
rule process_script:
    ...
    shell:
        ...
        singularity exec -B {params.volume_name}:/data {params.volume_name}/images/rename_gff.sif /bin/bash -c "cd /data && python /data/scripts/renameGff.py -g /data/{output.uniq_gff} -t {params.species_abbreviation} > {output.final_gff}"
        """
